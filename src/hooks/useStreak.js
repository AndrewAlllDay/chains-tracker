...
export const useStreak = (history = []) => {
    ...
    const consecutiveDays = useMemo(() => {
        ...
        // FILTER: Include both Practice AND League sessions
        const activeHistory = history.filter(s => s.type === 'PRACTICE' || s.type === 'LEAGUE');
        if (activeHistory.length === 0) return 0;

        // Get unique dates and sort them descending (newest first)
    ...
    // 3. Calculate the milestone message (Text only, no Lucide components here)
